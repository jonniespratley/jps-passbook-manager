...
module.exports = function(config, app) {
	...
	/**
	 * I handle CRUD operations on the Registrations table
	 */
	...
	/**
	 * I handle all routes on a device, such as getting serial numbers for device,
	 * registering a device and unregistering a device
	 */
	router.route(config.baseUrl + '/devices/:deviceLibraryIdentifier/registrations/:passTypeIdentifier/:serialNumber?').all(function(req, res, next){
		next();
	})
	.get(function(req, res, next){
	...
	.post(jsonParser, registerPass)
	.delete(unregisterPass);
	...
	router.get(config.baseUrl + '/passes/:passTypeIdentifier/:serialNumber?', function(req, res, next) {
		...
		if (req.params.passTypeIdentifier && req.params.serialNumber) {
			...
			util.log('Find pass by' + util.inspect(obj, {
				colors : true
			}));
		} else {
			res.status(404).send({
	...
	router.get(config.baseUrl + '/passes/:passTypeIdentifier/:serialNumber/export', function(req, res, next) {
		...
		if (req.params.passTypeIdentifier && req.params.serialNumber) {
			...
			util.log('Export pass by' + util.inspect(obj, {
				colors : true
			}));
