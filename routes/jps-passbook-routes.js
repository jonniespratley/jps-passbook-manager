...
module.exports = function(config, app) {
	...
	router.route(config.baseUrl + '/:col*').all(function(req, res, next) {
		console.log(req.params.col + ' route');
		next();
	...
	router.get(config.baseUrl + '/passes/:passTypeIdentifier/:serialNumber?', function(req, res, next) {
		var obj = {};
		if (req.params.passTypeIdentifier) {
			obj = {
				passTypeIdentifier : req.params.passTypeIdentifier,
				serialNumber : req.params.serialNumber
			};
	...
	app.use(function(req, res, next) {
		console.log(chalk.green('[jps-passbook-routes][%s] - %s - %s %s %s %s'), Date.now(), req.method, req.url, JSON.stringify(req.query), req.body, JSON.stringify(req.params));
		next();
	});
