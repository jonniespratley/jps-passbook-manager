...
function SignPass(opts /*pass_url, cert_url, cert_password, key, wwdr_cert_path, output_url, compress, tmpdir*/ ) {
	...
	assert(wwdr_intermediate_certificate_path, 'has wwdr');
	...
	function rename_pass_file(cb) {
		logger('rename_pass_file', 'from', zip_url);
	...
	function copy_to_dest(cb) {
		logger('copy_to_dest', 'from', pkpass_url);
		logger('copy_to_dest', 'to', options.outputFilename);
		fs.copy(pkpass_url, options.outputFilename, function(err) {
...
/*
 ...
 $ openssl pkcs12 -in cert.p12 -nocerts -out key.pem
*/
SignPass.createPems = function(p12, pass, callback) {
	let _out = [],
		cmd1 = {},
		cmd2 = {},
		_outCert,
	...
	function _certCmdExec(cb) {
		child_process.execSync(cmd1.cmd);
		cb(null, cmd1.cmd);
	}

	function _keyCmdExec(cb) {
		child_process.execSync(cmd2.cmd);
		cb(null, cmd2.cmd);
	}
	...
	async.series([
		...
		logger('createPems', 'result', result);
		if (callback) {
			callback(err, result);
		}
	});
