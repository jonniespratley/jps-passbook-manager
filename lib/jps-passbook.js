...
module.exports = function jpsPassbook(program) {
	...
	let logger = program.getLogger('jpsPassbook');
	let db = program.db;
	...
	function getPassCerts(id, cb) {
		...
		return new Promise(function(resolve, reject) {
			db.findOne({
				...
				log.info('getPassCerts-success', resp.cert);
				cb(null, resp);
				resolve(resp);
			}).catch(function(err) {
				log.error('getPassCerts', err);
				cb(err);
				reject(err);
			});
		});
		...
		getPassCerts(pass.passTypeIdentifier, function(err, _certs) {
			...
			signpass.signPromise().then(function(resp) {
				...
				savePass(pass, function(err, p) {
					logger('savePass', 'success', p._id);
					if(err){
						log.error('savePass', err);
					}
					//log.info('savePass', p._id);
					callback(null, resp);
				});
	...
	function createPass(pass, cb) {
		...
		return async.waterfall([
				...
				function(_type, _path, callback) {
					logger('createPass', '2. Copy assets', _type, _path);
	...
	return {
		...
		createPassPromise: function(pass) {
			log.info('createPassPromise', pass._id);
			return new Promise(function(resolve, reject) {
		...
		batchCreatePasses: function(passes){
			var self = this;
			var out = [];
			passes.forEach(function(p){
				out.push(self.createPassPromise(p));
			});
			return Promise.all(out);
		},
		batchSignPasses: function(passes){
			var self = this;
			...
			return new Promise(function(resolve, reject){
				...
				passes.forEach(function(p){
					_.defer(function(){
						self.signPassPromise(p).then(function(resp){
							_out.push(resp);
							_done();
						}).catch(reject);
					});
		...
		signpassValidation: function(pkpassFilename){
			assert(pkpassFilename, 'has pkpass filename');
			var bin = path.resolve(__dirname, '../bin/signpass');
			return shell(`${bin} -v ${pkpassFilename}`);
		}
	};
};
