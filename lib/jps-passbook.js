...
module.exports = function (_, fs, assert, path, async, Logger, db, config, Utils, SignPass, Pass, Passes) {
	...
	/**
	 * @description Get pass type identifier and invoke callback on success/error.
	 * @param id
	 * @param cb
	 */
	function getPassTypeIdentifier(id, cb) {
		db.findOne({
	...
	/**
	 * @description Get pass type identifier and resolve promise on success.
	 * @param id
	 * @returns {*}
	 */
	function getPassTypeIdentifierPromise(id) {
		return new Promise(function (resolve, reject) {
			getPassTypeIdentifier(id, function (err, resp) {
				if (err) {
					reject(err);
				} else {
					resolve(resp);
	...
	/**
	 * @description Save pass Type id to database, and create pems.
	 * @example
	 * @param {Object} obj An object with ..
	 * @param cb
	 * @returns {*}
	 */
	...
	function signPass(pass, callback) {
		...
		getPassTypeIdentifier(pass.passTypeIdentifier, function (err, _certs) {
			logger('getPassTypeIdentifier', _certs._id);
			if (err) {
				callback(err);
			}
	...
	function createPassPromise(pass) {
		return new Promise(function (resolve, reject) {
	...
	function validatePassPromise(pass) {
		return new Promise(function (resolve, reject) {
	...
	/**
	 * @description Perform a batch operation on an array of pass objects.
	 * @param action {String} The name of action. (create, sign, validate)
	 * @param passes {Array} An array of Pass objects.
	 * @returns {*}
	 */
	function batchPromise(action, passes) {
		return new Promise(function (resolve, reject) {
			let _resp = [];
			...
			let _doBatch = function (_pass) {
				switch (action) {
					case 'create':
						createPassPromise(_pass).then(function (p) {
						...
						break;
					case 'sign':
						signPassPromise(_pass).then(function (p) {
							_resp.push(p);
							_done();
						}).catch(function (err) {
							reject(err);
						...
						break;
					case 'validate':
						validatePassPromise(_pass).then(function (p) {
							_resp.push(p);
							_done();
						}).catch(function (err) {
							reject(err);
	...
	return {
		...
		getPassTypeIdentifierPromise: getPassTypeIdentifierPromise,
