		...
		post_device_registration: function(req, res, next) {
			...
			if (!authentication_token) {
				...
				program.db.get(registration._id).then(function(pass) {
					...
					logger('post_device_registration','found existing registration', registration._id);

					res.status(200).json(pass);
				}).catch(function(err) {
					// No registration found, lets add the device
					logger( 'post_device_registration', 'registration not found', 'inserting', registration._id);
