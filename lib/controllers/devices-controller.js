...
module.exports = function (program) {
	...
	return {
		...
		post_device_registration: function (req, res, next) {
			...
			logger('Registration request', ` Authorization = ${authentication_token}, device_id = ${device_id}, pass_type_id = ${pass_type_id}, serial_number = ${serial_number}, push_token = ${push_token}`);
			...
			if (!authentication_token) {
				...
				let uuid = 'registration-' + device_id + '-' + serial_number;
				let registration = {
					_id: uuid,
		...
		delete_device_registration: function (req, res, next) {
			...
			if (!authentication_token) {
				...
				program.db.get(uuid).then(function (pass) {
					...
					if (`ApplePass ${pass.authenticationToken}` === authentication_token) {
						logger('Pass and authentication token match.');
						program.db.remove(uuid).then(function (resp) {
							res.status(200).json(resp);
						});
					} else {
						logger('Pass and authentication token DO NOT match.');
						res.status(401).json({
