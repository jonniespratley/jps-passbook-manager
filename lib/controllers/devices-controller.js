...
module.exports = function(program) {
	...
	return {
		...
		post_device_registration: function(req, res, next) {
			...
			if (!authentication_token) {
				...
				try {
					...
					program.db.put(registration._id).then(function(reg) {
						...
						program.db.post(registration).then(function(resp) {
		...
		delete_device_registration: function(req, res, next) {
			logger('delete_device_registration');

			let authentication_token = `ApplePass ${req.get('Authorization')}`;
			let device_id = req.params.device_id;
			let pass_type_id = req.params.pass_type_id;
			let serial_number = req.params.serial_number;
			...
			if (!authentication_token) {
				...
				program.db.get(uuid).then(function(pass) {
					logger('delete_device_registration', 'found', pass._id);

					if (`${pass.authenticationToken}` === authentication_token) {

						logger('delete_device_registration', 'pass.authenticationToken =', pass.authenticationToken);
						logger('delete_device_registration', 'req.authenticationToken =', authentication_token);
