	...
	return {
		...
		post_device_registration: function(req, res, next) {
			...
			if (!authentication_token) {
				...
				try {
					...
					//todo find pass by serialNumber
					Passes.findPassBySerial(serial_number).then(function(pass){
		...
		delete_device_registration: function(req, res, next) {
			...
			if (!authentication_token) {
				...
				program.db.get(registration._id).then(function(reg) {
					tokensMatch = (authentication_token === reg.device.authorization);
					...
					if (tokensMatch) {
						...
						logger('delete_device_registration', 'req.authenticationToken =', authentication_token);
						logger('delete_device_registration', 'device token =', reg.auth_token);
